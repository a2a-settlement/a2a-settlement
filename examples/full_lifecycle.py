...
def _run_provider(provider_account_id: str, provider_api_key: str) -> None:
    ...
    class ProviderExecutor(AgentExecutor):
        async def execute(self, context, event_queue) -> None:  # type: ignore[override]
            ...
            completed = TaskStatus(state=TaskState.completed)
            await event_queue.enqueue_event(
                Task(
                    id=context.task_id,
                    contextId=context.context_id,
        ...
        async def cancel(self, context, event_queue) -> None:  # type: ignore[override]
            canceled = TaskStatus(state=TaskState.canceled)
            await event_queue.enqueue_event(
                Task(
                    id=context.task_id,
                    contextId=context.context_id,
    ...
    settlement_ext = build_settlement_extension(
        ...
        pricing={
            "sentiment-analysis": {"baseTokens": 10, "model": "per-request", "currency": "ATE"}
        },
        reputation=0.5,
        availability=0.95,
        required=True,
...
async def _run_requester(provider_account_id: str, requester_api_key: str) -> None:
    ...
    async with httpx.AsyncClient() as httpx_client:
        ...
        msg = Message(
            messageId="msg-1",
            role="user",
            parts=[TextPart(text="Analyze sentiment of: profits are up, churn is down.")],
            metadata=md,
        )
        ...
        print("Requester balance:", exchange.get_balance())
